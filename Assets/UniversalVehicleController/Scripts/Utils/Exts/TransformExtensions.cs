...
#if UNITY_EDITOR
		[MenuItem ("CONTEXT/Transform/Move to surface")]
		public static void MoveToSurface (MenuCommand command)
		{
			var transform = (Transform)command.context;
			RaycastHit hit;
			if (Physics.Raycast (new Ray (transform.position, Vector3.down), out hit, 1000))
			{
				var pos = hit.point;
				pos.y -= 0.02f;
				transform.position = pos;

				transform.rotation = Quaternion.FromToRotation (Vector3.up, hit.normal) * Quaternion.Euler (0, transform.eulerAngles.y, 0);
			}
		}
#endif
	}
